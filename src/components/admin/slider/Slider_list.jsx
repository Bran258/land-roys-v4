  ...
  return (
    ...
    >
      <div className="w-16 h-12 rounded-lg overflow-hidden bg-gray-100 shrink-0 flex items-center justify-center">
        {slide.url_image ? (
          <img src={getPublicImageUrl(slide.url_image)} alt="Slide" className="w-full h-full object-cover" />
        ) : (
...
// Lista principal
const SliderList = ({ slides, selectedId, onSelect, onReorder, onAddSlide, onSaveOrder }) => {
  ...
  return (
    <div className="p-6 bg-white border-r border-gray-100 min-h-screen">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h2 className="text-xl font-bold text-slate-800">Slides</h2>
          <p className="text-xs text-gray-400">Máximo 5 slides permitidos</p>
        </div>
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`flex items-center gap-1 text-[10px] font-black px-3 py-2 rounded-full
              ${slides.length >= 5 ? "bg-gray-300 text-gray-700 cursor-not-allowed" : "bg-yellow-400 text-black"}`}
          >
            <Plus size={14} /> Nuevo
          </button>

          <button
            onClick={() => onSaveOrder(slides)}
            className="flex items-center gap-1 bg-green-500 text-white text-[10px] font-black px-3 py-2 rounded-full"
          >
      ...
      {modalOpen && <SlideModal onClose={() => setModalOpen(false)} onSave={onAddSlide} />}
    </div>
  );
};
